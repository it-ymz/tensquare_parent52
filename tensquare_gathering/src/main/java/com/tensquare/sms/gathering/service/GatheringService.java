package com.tensquare.sms.gathering.service;
...
import com.tensquare.sms.gathering.dao.GatheringDao;
...
public class GatheringService {
	 ...
	 */
	/**
	 *  @CacheEvict  是从spring boot中进行删除缓冲
	 *  @Cacheable注解描述
	 *  Cacheable中有两个参数(value = , key = )
	 *   value表示的全局名称  意思是说 这个缓冲在spring cache中的名称，它里面可以有多个key
	 *   key表示的是  在spring cache这个缓冲中具体的一个标识符  可以通过这个标识符得到相对应的值
	 *
	 *   在spring cache的缓冲中是这样执行的：
	 *     当来到一个请求，那么它会根据传来的唯一标识符先去spring cache中进行查询，当缓冲中的返回类型是Gathering
	 *     那么就直接从缓冲中进行取值
	 *
	 *    spring boot缓冲
	 *     优点：方便使用
	 *     缺点：spring boot的cacheable缓冲是无法设置过期时间的  而spring data中的Redis是可以设置过期时间的
	 *
	 *   spring boot cacheable缓冲在java虚拟机当中
	 */
	@Cacheable(value = "gathering",key = "#id")   //这个是往缓冲中进行存
